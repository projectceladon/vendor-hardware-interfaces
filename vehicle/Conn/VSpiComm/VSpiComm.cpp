...
VSpiComm::VSpiComm(MessageProcessor* messageProcessor)
    : CommConn(messageProcessor), mSpiFd(-1), mSpiDevice("/dev/spidev0.0"), mMode(SPI_MODE_0), mBitsPerWord(8), mSpeed(100000) {
...
std::vector<uint8_t> VSpiComm::read() {
    uint8_t rxBuffer[MAX_BUFFER_SIZE] = {0}; // Buffer for rx_buf with max size 128
    uint8_t dummyTxBuffer[MAX_BUFFER_SIZE] = {0}; // Dummy buffer for tx_buf with max size 128
...
int VSpiComm::write(const std::vector<uint8_t>& data) {
    ...
    uint8_t dummyRxBuffer[MAX_BUFFER_SIZE] = {0}; // Dummy buffer for rx_buf
    uint8_t txBuffer[MAX_BUFFER_SIZE] = {0}; // Buffer to store size and data
